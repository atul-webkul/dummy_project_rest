	...
	if ( $_FILES['product_info_csv']['name'] && $_FILES['product_image_zip']['name'] ) {
		...
		if ( isset($_POST['admin_mass_upload_nonce']) && !empty( $_POST['admin_mass_upload_nonce'] ) ) {
	 		...
	 		if( wp_verify_nonce( $_POST['admin_mass_upload_nonce'], 'admin_mass_upload_action' )){
				...
				$csv_type = $_FILES['product_info_csv']['type'];

				$zip_type = $_FILES['product_image_zip']['type'];

				$csv_size = $_FILES['product_info_csv']['size'];

			    // Check file size
				if ($csv_size > 10000000) {
				    echo "<div class='csv-error'>Sorry, your file is too large.</div>";
				    $uploadOk = 0;
				}
				...
				$allowed_csv_type = array( "text/csv", "application/octet-stream", "application/vnd.ms-excel", "application/excel", "application/csv" );
...
if (!is_admin()) {
	...
	echo '<h2 style="display:inline-block;">Mass Upload</h2>';
