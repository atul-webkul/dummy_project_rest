...
if ( cpm_user_can_access( $project_id, 'tdolist_view_private' ) ) {
	$lists = $task_obj->get_task_lists( $project_id, true, 0, true, true );
} else {
	$lists = $task_obj->get_task_lists( $project_id, false, 0, true, true );
}
...
if ( $lists ) {
	...
	foreach ( $lists['lists'] as $list ) {
		...
		$task_list_complete = CPM_Task::getInstance()->get_completeness( $list->ID, $project_id );

		if ( $task_list_complete['total'] > 0 ) {
			$percentage = ( 100 * $task_list_complete['completed'] ) / $task_list_complete['total'];
		} else {
			$percentage = 0;
		}
		...
		if ( count( $tasks ) ) {
			foreach ( $tasks['pending'] as $task ) {
				...
				$assigned = get_post_meta( $task->ID, '_assigned' );
				...
				if ( is_array( $assigned ) ) {
					...
					foreach ( $assigned as $key => $user_id ) {
						...
						$avatar_single .= get_avatar( $user_id, 16 );
					}
					$avatar[] = array(
						'task_id' => $task->ID,
						'avatar'  => $avatar_single,
					);
				} else {
					$avatar[] = array(
						'task_id' => $task->ID,
						'avatar'  => get_avatar( $assigned, 16 ),
					);
				}
...
<script>
	...
	var cproject_id = <?php echo $project_id; ?> ;
</script>
