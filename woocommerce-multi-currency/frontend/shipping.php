...
//		$this->settings = new WOOMULTI_CURRENCY_Data();
	...
	public function woocommerce_package_rates( $methods, $package ) {
		...
		if ( count( array_filter( $methods ) ) ) {
			foreach ( $methods as $k => $method ) {
				if ( in_array( $method->method_id, array(
					'betrs_shipping',
					'printful_shipping',
					'easyship',
	...
	protected function evaluate_cost( $sum, $args = array() ) {
		...
		$this->fee_cost = $args['cost'] / wmc_get_price( 1, $this->settings->get_current_currency() );
		// Expand shortcodes.
		...
		$sum = do_shortcode(
			str_replace(
				...
				array(
					$args['qty'],
					$args['cost']/wmc_get_price( 1, $this->settings->get_current_currency() ),
				),
				$sum
			)
		);
