...
class WOOMULTI_CURRENCY_Frontend_Price {
	...
	public function add_approximately_price( $html_price, $product ) {
		...
		if ( $this->settings->get_auto_detect() == 2 ) {
			...
			$html_price .= '<div class="wmc-approximately">' . esc_html__( 'Approximately', 'woocommerce-multi-currency' ) . ': ' . $formatted_price . $max_price . '</div>';
	...
	public function init() {
		...
		} else if ( ! isset( $_REQUEST['custom-page'] ) ) {//check ! isset( $_REQUEST['custom-page'] ) to fix looping checkout error with theme Handmade
			if ( ( $allow_multi && $id == get_option( 'woocommerce_checkout_page_id', 0 ) ) ) {
				$this->settings->set_current_currency( $checkout_currency, false );
			} elseif ( ( ( $this->settings->enable_cart_page() && $id == get_option( 'woocommerce_cart_page_id', 0 ) ) ) ) {
				$this->settings->set_current_currency( $checkout_currency, false );
			} elseif ( $old_currency && $old_currency != $current_currency ) {
				$this->settings->set_current_currency( $old_currency, false );
			}
