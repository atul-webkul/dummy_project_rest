...
class CPM_Pro_API {
	...
	function check_authentication( $authentication ) {
		...
		if ( ! isset( $_SERVER['HTTP_CPMKEY'] ) ) {
			return new WP_Error( 'invalid_key', __( 'API key does not exist!', 'cpm-pro' ), array( 'status' => 404 ) );
		}

		if ( ! isset( $_SERVER['HTTP_CPMSECRET'] ) ) {
			return new WP_Error( 'invalid_secret', __( 'Secret key does not exist!', 'cpm-pro' ), array( 'status' => 404 ) );
		}

		$key = $this->get_user_by_consumer_key( $_SERVER['HTTP_CPMKEY'] );

		if ( ! $key ) {
			return new WP_Error( 'invalid_key', __( 'API key does not match!', 'cpm-pro' ), array( 'status' => 404 ) );
		}

		$secret = $this->is_consumer_secret_valid( $key, $_SERVER['HTTP_CPMSECRET'] );

		if ( ! $secret ) {
			return new WP_Error( 'invalied_secret', __( 'Secret key does not match!', 'cpm-pro' ), array( 'status' => 404 ) );
		}
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			...
			?>
				<table class="form-table">
					<tbody>
						<tr>
							<th><label for="cpm_api_keys"><?php _e( 'Project API Keys', 'cpm-pro' ); ?></label></th>
							<td>
								<?php if ( empty( $user->cpm_api_consumer_key ) ) : ?>
									<label for="cpm_generate_api_key">
										...
										<?php _e( 'Generate API Key', 'cpm-pro' ); ?>
									</label>
								<?php else : ?>
									<div class="api-keys-wrapper">
										<strong><?php _e( 'Consumer Key:', 'cpm-pro' ); ?></strong><br /><code id="cpm_api_consumer_key"><?php echo $user->cpm_api_consumer_key; ?></code><br/><br />
										<strong><?php _e( 'Consumer Secret:', 'cpm-pro' ); ?></strong><br /><code id="cpm_api_consumer_secret"><?php echo $user->cpm_api_consumer_secret; ?></code><br/>
									</div>
