...
class CPM_Pro_Updates {
     ...
     */
    function admin_menu() {
        add_submenu_page( 'cpm_projects', __( 'Updates', 'cpm-pro' ), __( 'Updates', 'cpm-pro' ), 'activate_plugins', 'cpm_updates', array($this, 'plugin_update') );
    }
    ...
    function license_check_notice() {
        ...
        $error = __( 'Please activate your copy', 'cpm-pro' );
        ...
        ?>
        <div class="error">
            <p><strong><?php _e( 'WP Project Manager Error:', 'cpm-pro' ); ?></strong> <?php echo $error; ?></p>
        </div>
        <?php
    }
    ...
    function plugin_update() {
        ...
        if ( isset( $_POST['submit'] ) ) {
            if ( empty( $_POST['email'] ) ) {
                $errors[] = __( 'Empty email address', 'cpm-pro' );
            }

            if ( empty( $_POST['license_key'] ) ) {
                $errors[] = __( 'Empty license key', 'cpm-pro' );
            }
            ...
            if ( !$errors ) {
                ...
                echo '<div class="updated"><p>' . __( 'Settings Saved', 'cpm-pro' ) . '</p></div>';
            }
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e( 'Plugin Activation', 'cpm-pro' ); ?></h2>
            ...
            if ( !isset( $license_status->activated ) || $license_status->activated != true ) {
                ...
                <form method="post" action="">
                    <table class="form-table">
                        <tr>
                            <th><?php _e( 'E-mail Address', 'cpm-pro' ); ?></th>
                            <td>
                                <input type="email" name="email" class="regular-text" value="<?php echo esc_attr( $email ); ?>" required>
                                <span class="description"><?php _e( 'Enter your purchase Email address', 'cpm-pro' ); ?></span>
                            </td>
                        </tr>
                        <tr>
                            <th><?php _e( 'License Key', 'cpm-pro' ); ?></th>
                            <td>
                                <input type="text" name="license_key" class="regular-text" value="<?php echo esc_attr( $key ); ?>">
                                <span class="description"><?php _e( 'Enter your license key', 'cpm-pro' ); ?></span>
                            </td>
                        </tr>
                    </table>
            ...
            <?php } else {
                ...
                <div class="updated">
                    <p><?php _e( 'Plugin is activated', 'cpm-pro' ); ?></p>
                </div>

                <form method="post" action="">
                    <?php submit_button( __( 'Delete License', 'cpm-pro' ), 'delete', 'delete_license' ); ?>
                </form>
