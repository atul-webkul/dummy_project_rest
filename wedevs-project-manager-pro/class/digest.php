...
class CPM_Pro_Digest {
    ...
    function __construct() {
        ...
        add_action( 'init', array( $this, 'do_initial' ) );
    }

    function do_initial() {
        $this->_link_option = cpm_get_option( 'email_url_link', 'cpm_mails' );
        $this->_page_id     = cpm_get_option( 'project', 'cpm_page' );
    }
    ...
    function get_all_users() {
        ...
        foreach ( $users as $user_id => $user ) {
            ...
            $title           = sprintf( __( '[%s] - Project Manager Daily Digest (%s)', 'cpm-pro' ), get_bloginfo( 'name' ), $date );
    ...
    function table_start() {
        ...
        <html>
            <head>
                ...
                <title><?php _e( 'Project Daily Digest', 'cpm-pro' ); ?></title>
            </head>
            <body style="background-color: #f4f4f4; font-family: 'Helvetica Neue',Helvetica,Arial,sans-serif; font-size:14px; color: #666; margin: 0; padding-top: 30px;">
                <div style="width: 600px; padding: 20px; border: 1px solid #eee; margin: 0 auto; background-color: #fff;">
                    <table width="100%" bgcolor="#fff" style="border-collapse: collapse; border: none;">
                        <tr>
                            <td style="font-weight: bold; font-size: 22px; padding-bottom: 5px;"><?php _e( 'Project Daily Digest', 'cpm-pro' ); ?></td>
                        </tr>
                        <tr>
                            <td style="font-size: 11px; color: #9d9d9d; padding: 0;"><?php _e( 'Project items due or added or completed in the last 24 hours', 'cpm-pro' ); ?></td>
                        </tr>
                        <?php
                    }
    ...
    function table_close() {
                ...
                <div style="width: 600px; padding: 20px; margin: 0 auto;">
                    <span style="font-size: 11px; text-align: center; display: block; font-style: italic;">
                        <?php printf( __( 'This is an automated email from %s, powered by %s.', 'cpm-pro' ), $site_link, $plug_link ); ?>
                    </span>
                </div>
            </body>
        </html>
    ...
    function co_worker_per_day_activity( $user_id, $project_id, $co_worker_tasks, $co_worker_complete_tasks, $co_worker_due_tasks ) {
            ...
            if ( isset( $co_worker_tasks[$project_id][$user_id] ) && $co_worker_tasks[$project_id][$user_id] ) {
                ...
                ?>
                <tr>
                    <td style="font-weight: bold; color:#336364; padding: 15px;  border-top: 0; border-bottom: 0"><?php _e( 'New to-do items:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
            ...
            if ( isset( $co_worker_complete_tasks[$project_id][$user_id] ) && $co_worker_complete_tasks[$project_id][$user_id] ) {
                ...
                ?>
                <tr>
                    <td style="color:#1993c4; padding-left: 20px;"><?php _e( 'Completed to-do items:', 'cpm-pro' ); ?></td>
                </tr>
            <?php
            foreach ( $lists as $list_id => $tasks ) {
            ...
            if ( isset( $co_worker_due_tasks[$project_id][$user_id] ) && $co_worker_due_tasks[$project_id][$user_id] ) {
                ...
                ?>
                <tr>
                    <td style="color:#1993c4; padding-left: 20px;"><?php _e( 'Due to-do items:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
                foreach ( $lists as $list_id => $tasks ) {
    ...
    function manager_per_day_activity( $user_id, $project_id, $manager_tasks, $manager_complete_tasks, $manager_due_task, $manager_milestone, $manager_complete_milestone ) {
            ...
            if ( isset( $manager_tasks[$project_id] ) && $manager_tasks[$project_id] ) {
                ...
                ?>
                <tr>
                    <td style="font-weight: bold; color:#336364; padding: 15px;  border-top: 1px solid #f5f5f5; border-bottom: 0"><?php _e( 'New to-do items:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
                foreach ( $lists as $list_id => $tasks ) {
                    $task = reset( $tasks );
            ...
            if ( isset( $manager_complete_tasks[$project_id] ) && $manager_complete_tasks[$project_id] ) {
                ...
                ?>
                <tr>
                    <td style="font-weight: bold; color:#336364; padding: 15px; border-top: 1px solid #f5f5f5; border-bottom: 0"><?php _e( 'Completed to-do items:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
                foreach ( $lists as $list_id => $tasks ) {
            ...
            if ( isset( $manager_due_task[$project_id] ) && $manager_due_task[$project_id] ) {
                ...
                ?>
                <tr>
                    <td style="font-weight: bold; color:#336364; padding: 15px; border-top: 1px solid #f5f5f5; border-bottom: 0"><?php _e( 'Due to-do items:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
                foreach ( $lists as $list_id => $tasks ) {
            ...
            if ( isset( $manager_milestone[$project_id] ) && $manager_milestone[$project_id] ) {
                ?>
                <tr>
                    <td style="font-weight: bold; color:#336364; padding: 15px; border-top: 1px solid #f5f5f5; border-bottom: 0"><?php _e( 'New milestones:', 'cpm-pro' ); ?></td>
                </tr>
                <?php
    ...
    function cpm_url_single_tasklist( $project_id, $list_id ) {
        if ( $this->_link_option == 'frontend' ) {
            ...
            $url = sprintf( '%s?page=cpm_projects&tab=task&action=index&pid=%d#/list/%d', admin_url( 'admin.php' ), $project_id, $list_id );
        }
    ...
    function cpm_url_single_task( $project_id, $list_id, $task_id ) {
        if ( $this->_link_option == 'frontend' ) {
            ...
            $url = sprintf( '%s?page=cpm_projects&tab=task&action=index&pid=%d#/list/%d/task/%d', admin_url( 'admin.php' ), $project_id, $list_id, $task_id );
        }
    ...
    function complete_task_html_( $task ) {
            ...
            if ( $task->completed == '1' && $task->completed_by ) {
                ...
                ?>
                    ...
                    <?php //printf( __( '(Completed by %s on %s)', 'cpm-pro' ), cpm_url_user( $task->completed_by ), $completion_time )  ?>
                    </span>
                <?php } ?>
            </div>
    ...
    function complete_task_html( $task ) {
            ...
            if ( $task->complete == '1' ) {
                ...
                ?>
                <span style="color: #999; font-size: 11px;">
                <?php printf( __( '(Completed by %s)', 'cpm-pro' ), $user->display_name ) ?>
                </span>
            <?php } ?>
            <?php
        }
