...
class CPM_Pro_Report {
    ...
    public function filtermilestone() {
        ...
        if ( $milestones ) {
            $milestone_array[] = array(
                'text' => __( "Select a milestone", 'cpm-pro' ),
                'val'  => ''
            );
            ...
            $milestone_array[] = array(
                'text' => __( "No milestone on this Project", "cpm-pro" ),
                'val'  => ''
            );
        }
    ...
    public function overdue_task() {
        ...
        $response['reporttitle']      = __( 'Overdue Task', 'cpm-pro' );
        $response['selectedproject']  = __( 'All Project', 'cpm-pro' );
        $response['selectedcoworder'] = __( 'All Coworker', 'cpm-pro' );
        $response['extrahead']        = '';
    ...
    public function complete_task() {
        ...
        $response['reporttitle']      = __( 'Completed Task', 'cpm-pro' );
        $response['selectedproject']  = __( 'All Project', 'cpm-pro' );
        $response['selectedcoworder'] = __( 'All Coworker', 'cpm-pro' );
        $response['extrahead']        = '';
        $response['timetracker'] = $this->_timetracker ;
    ...
    public function useractivity() {
        ...
        $response['reporttitle']      = __( 'User Activity Report', 'cpm-pro' );
        $response['selectedproject']  = __( 'All Project', 'cpm-pro' );
        $response['selectedcoworder'] = '';
        $response['extrahead']        = __( '<b> Rnage </b>', 'cpm-pro' ) . "<br/> ". mysql2date( $date_format, $form_date) ." to  ".mysql2date( $date_format, $to_date);
        $response['timetracker'] = $this->_timetracker ;
    ...
    public function taskbyproject() {
        ...
        $response['reporttitle']      = __( 'All Task by Project', 'cpm-pro' );
        $response['selectedproject']  = __( 'All Project', 'cpm-pro' );
        $response['selectedcoworder'] = false;
        $response['timetracker'] = $this->_timetracker ;
        $status_head = __( "Task Status : All Task", 'cpm-pro' );
    ...
    public function taskbymilestone() {
        ...
        $response['reporttitle']      = __( 'All Task by Milestone', 'cpm-pro' );
        $response['selectedproject']  = __( 'All Project', 'cpm-pro' );
        $response['selectedcoworder'] = __( 'All Coworker', 'cpm-pro' );
        $response['timetracker'] = $this->_timetracker ;
        $status_head = "";
        ...
        $response['extrahead'] = "<b>" . __( 'Milestone', 'cpm-pro' ) . "<br/>" . $smilestone . "</b>";
    ...
    public function unassignedtask() {
        ...
        $response['reporttitle']     = __( 'All Unassigned Task', 'cpm-pro' );
        $response['selectedproject'] = __( 'All Project', 'cpm-pro' );
        $response['timetracker'] = $this->_timetracker ;
    ...
    public function render_report_data( $task_list ) {
        ...
        foreach ( $task_list as $task ) {
            ...
            if ( ! empty( $task->assign_user ) AND  $task->assign_user != -1) {
                ...
                $asigned_user = __( 'Not assigned', 'cpm-pro' );
            }
            ...
            $task_obj = array(
                ...
                'task_time'       => $ttime,
                'start_date'      => ($task->start_date != '0000-00-00 00:00:00' ) ? mysql2date( $date_format, $task->start_date ) : __( 'Not set', 'cpm-pro' ),
                'due_date'        => ($task->due_date != '0000-00-00 00:00:00' ) ? mysql2date( $date_format, $task->due_date ) : __( 'No due date', 'cpm-pro' ),
                'complete_date'   => ($task->complete_date != '0000-00-00 00:00:00' ) ? mysql2date( $date_format, $task->complete_date ) : __( 'Incomplete', 'cpm-pro' ),
                'complete_status' => $task->complete_status == '1' ? __( 'Complete', 'cpm-pro' ) : __( 'Incomplete', 'cpm-pro' ),
            );
...
//echo '<pre>'; print_r( $data ); echo '</pre>'; die();
            ...
            $str = array( __( 'Report Title : ', 'cpm-pro' ), $data['reporttitle'] );
            fputcsv( $fp, $str );

            $str = array( __( 'Project for : ', 'cpm-pro' ), $data['selectedproject'] );
            fputcsv( $fp, $str );

            $str = array( __( 'Co Worker : ', 'cpm-pro' ), $data['selectedcoworder'] );
            fputcsv( $fp, $str );
            ...
            if ( $this->_timetracker ) {
                $header = array( __( 'Task Name', 'cpm-pro' ), __( 'Assign to', 'cpm-pro' ), __( 'Assign date', 'cpm-pro' ), __( 'Due Date', 'cpm-pro' ), __( 'Complete Date', 'cpm-pro' ), __( 'Time', 'cpm-pro' ), __( 'Status', 'cpm-pro' ) );
            } else {
                $header = array( __( 'Task Name', 'cpm-pro' ), __( 'Assign to', 'cpm-pro' ), __( 'Assign date', 'cpm-pro' ), __( 'Due Date', 'cpm-pro' ), __( 'Complete Date', 'cpm-pro' ),  __( 'Status', 'cpm-pro' ) );
            }

            foreach ( $data['output'] as $fields ) {
                $str = array( __( 'Project : ', 'cpm-pro' ), $fields['project_name'] );
                fputcsv( $fp, $str );
                ...
                foreach ( $fields['list'] as $list ) {
                    ...
                    foreach ( $list['task'] as $task ) {
                        if ( $this->_timetracker ) {
                            $time = $task['task_time']['hour'] .':'. $task['task_time']['minute'] .':'. $task['task_time']['second'];
                            
                            $str = array( 
                                strip_tags( $task['task_name'] ), 
                                strip_tags( $task['assignto'] ), 
                                $task['start_date'], 
                                $task['due_date'], 
                                $task['complete_date'], 
                                $time,
    ...
    public static function csv_generate( $data ) {
        ...
        if ( in_array( 'time', $data['filter'] ) ) {
            ...
            if ( $data['interval'] == 1 ) {
                $interval      = 'post_year';
                $interval_view = __( 'Year', 'cpm-pro' );
            } else if ( $data['interval'] == 2 ) {
                $interval      = 'post_month';
                $interval_view = __( 'Month', 'cpm-pro' );
            } else if ( $data['interval'] == 3 ) {
                $interval      = 'post_week';
                $interval_view = __( 'Week', 'cpm-pro' );
            }
        ...
        if ( in_array( 'time', $data['filter'] ) ) {
            foreach ( $items as $key => $item ) {
                ...
                foreach ( $item as $project_id => $projects ) {
                    ...
                    _e( 'Project Title: ', 'cpm-pro' ) . '  ';
                    echo $project->post_title . "\n";

                    foreach ( $projects as $list_id => $lists ) {
                        $list = get_post( $list_id );
                        _e( 'Task List Title: ', 'cpm-pro' ) . '  ';
                        echo $list->post_title . "\n";

                        $task_cell   = __( 'Task', 'cpm-pro' );
                        $assign_cell = __( 'Assign To', 'cpm-pro' );
                        $sdate_cell  = __( 'Start Date', 'cpm-pro' );
                        $edate_cell  = __( 'Due Date', 'cpm-pro' );
                        $status_cell = __( 'Status', 'cpm-pro' );
                        ...
                        foreach ( $lists as $task_id => $tasks ) {
                            ...
                            $status = $task->completed ? __( 'Completed', 'cpm-pro' ) : __( 'Incompleted', 'cpm-pro' );
                            echo $status . "\n";
                        }
    ...
    public static function report_generate( $data ) {
        ...
        add_filter( 'posts_join', array( cpmpro()->report, 'co_worker_table' ) );
        add_filter( 'posts_where', array( cpmpro()->report, 'co_worker_where' ) );
        add_filter( 'posts_fields', array( cpmpro()->report, 'select_field' ), 10, 2 );
        add_filter( 'posts_groupby', array( cpmpro()->report, 'posts_groupby' ) );

        $args    = apply_filters( 'cpm_report_args', $args );
        $results = new WP_Query( $args );

        remove_filter( 'posts_join', array( cpmpro()->report, 'co_worker_table' ) );
        remove_filter( 'posts_where', array( cpmpro()->report, 'co_worker_where' ) );
        remove_filter( 'posts_fields', array( cpmpro()->report, 'select_field' ), 10, 2 );
        remove_filter( 'posts_groupby', array( cpmpro()->report, 'posts_groupby' ) );
