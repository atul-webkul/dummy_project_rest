...
class CPM_Pro_Task {
    ...
    function task_count( $user_id, $range = false ) {
        ...
        $counts = array (
            __( 'Current', 'cpm-pro' )     => 0,
            __( 'Outstanding', 'cpm-pro' ) => 0,
            __( 'Completed', 'cpm-pro' )   => 0
        );

        foreach ( $task as $key => $obj ) {
            if ( ( empty( $obj->due_date ) || date( 'Y-m-d', strtotime( $obj->due_date ) ) >= date( 'Y-m-d', time() ) ) && $obj->complete_status != 1 ) {
                $counts[ __( 'Current', 'cpm-pro' ) ] += 1;
            }
            if ( !empty( $obj->due_date ) && date( 'Y-m-d', strtotime( $obj->due_date ) ) < date( 'Y-m-d', time() ) && $obj->complete_status != 1 ) {
                $counts[ __( 'Outstanding', 'cpm-pro' ) ] += 1;
            }

            if ( $obj->complete_status == 1 ) {
                $counts[ __( 'Completed', 'cpm-pro' ) ] += 1;
            }
        }
    ...
    function get_user_all_task( $user_id, $project_id ) {
        ...
        $que = "SELECT t.post_title as task, t.comment_count as comment_count, t.ID as task_id, tl.post_title as list, tl.ID as task_list_id,
                    ...
                    strday.meta_value as start_date, t.post_date as post_date
                FROM `$wpdb->posts` AS t
                LEFT JOIN $wpdb->posts AS tl ON t.post_parent = tl.ID
                LEFT JOIN $wpdb->posts AS p ON tl.post_parent = p.ID
                LEFT JOIN $wpdb->postmeta AS m ON m.post_id = t.ID
    ...
    function mytask_count( $user_id ) {
        ...
        $response[ __( 'Current', 'cpm-pro' ) ]     = $task[ __( 'Current', 'cpm-pro' ) ];
        $response[ __( 'Outstanding', 'cpm-pro' ) ] = $task[ __( 'Outstanding', 'cpm-pro' ) ];
        $response[ __( 'Completed', 'cpm-pro' ) ]   = $task[ __( 'Completed', 'cpm-pro' ) ];
    ...
    function mytask_date_chart( $user_id, $start_date, $end_date ) {
        ...
        $response[ 'ctask_list' ]  = array();
        $response[ 'cotask_list' ] = array();
        $date_list = array ();
    ...
    function user_overview( $user_id ) {
        ...
        include_once CPM_PRO_PATH . '/views/task/overview.php';
    }

    function user_activity( $user_id ) {
        include_once CPM_PRO_PATH . '/views/task/activity.php';
    }
    ...
    function taskhtmloutput( $project, $user_id, $tab ) {
        ...
        if ( empty( $project ) ) {
            printf( '<h2>%s</h2>', __( 'No Data Found!', 'cpm-pro' ) );
        }else {
            include_once CPM_PRO_PATH . '/views/task/taskhtml.php';
        }
    }

    function mytask_calender( $user_id ) {

        include_once CPM_PRO_PATH . '/views/task/mycalender.php';
    }
     ...
     * @return type
     */
    function get_user_activity( $user_id, $offset = 0 ) {
        ...
        if ( $offset != -1 ) {
            ...
            $from = " LIMIT  {$offset}, 20  ";
        }

        $comment_tale = $wpdb->prefix . "comments" ;

        $sql = " SELECT * FROM {$comment_tale}   WHERE  user_id = {$user_id} AND comment_type = 'cpm_activity'   ORDER BY comment_ID DESC  {$from}";

        $sql = apply_filters( 'cpm_mytask_user_activity_sql', $sql );
    ...
    function get_user_activity_total( $user_id ) {
        ...
        $count = $this->get_user_activity($user_id, -1);
