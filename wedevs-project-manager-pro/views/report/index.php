<div id="cpm-report" v-cloak>
    ...
    <h1 class="cpm-no-print report-page-title">
        <?php _e( 'Project Report', 'cpm-pro' ); ?>
        <a href="#" v-if="!dashboadmode" class="page-title-action" @click.prevent="switcreporthmode"><?php _e( 'Back to Report Dashboard', 'cpm-pro' ) ?></a>
        <a href="<?php echo cpm_report_advancesearch_url() ?>" v-if="dashboadmode" class="cpm-right cpm-advance-search" ><?php _e( 'Advanced Report', 'cpm-pro' ) ?></a>
    </h1>
        ...
        <div class="postbox">
            ...
            <ul>
                <li class="" v-on:click="showreport('overdue')">
                    ...
                    <h3> <?php _e( 'Overdue Tasks', 'cpm-pro' ) ?> </h3>
                    <p> <?php _e( 'Generate a report based on <strong>tasks</strong> which are <strong>pending</strong> beyond due dates.', 'cpm-pro' ) ?> </p>
                </li>
                <li class="" v-on:click="showreport('completetask')">
                    <img src="<?php echo CPM_URL ?>/assets/images/completed_task.svg" height="64" /><br/>
                    <h3><?php _e( 'Complete Task', 'cpm-pro' ) ?></h3>
                    <p><?php _e( 'Generate a report from <strong>tasks</strong> which were <strong>completed</strong>.', 'cpm-pro' ) ?> </p>
                </li>
                <li class="" v-on:click="showreport('useractivity')">
                    <img src="<?php echo CPM_URL ?>/assets/images/user_activity.svg" height="64" /> <br/>
                    <h3><?php _e( 'User Activities', 'cpm-pro' ) ?></h3>
                    <p><?php _e( 'Create a report based on an employee or <strong>all employee</strong> activity on <strong>tasks</strong>.', 'cpm-pro' ) ?> </p>
                </li>
                <li class=""  v-on:click="showreport('taskbyproject')">
                    <img src="<?php echo CPM_URL ?>/assets/images/taskby_project.svg" height="64" /> <br/>
                    <h3><?php _e( 'Project Task', 'cpm-pro' ) ?></h3>
                    <p><?php _e( 'Find out <strong>all tasks</strong> from your <strong>Project</strong>.', 'cpm-pro' ) ?> </p>

                </li>
                <li class=""  v-on:click="showreport('taskbymilestone')">
                    <img src="<?php echo CPM_URL ?>/assets/images/taskby_milestone.svg" height="64" /> <br/>
                    <h3> <?php _e( 'Task by Milestone', 'cpm-pro' ) ?> </h3>
                    <p> <?php _e( 'Browse   <strong>tasks</strong> reports according to <strong>Milestones</strong> (CSV exportable).', 'cpm-pro' ) ?></p>
                ...
                <li class="" v-on:click="showreport('unassignedtask')">
                    ...
                    <p><?php _e( 'Find out <strong>all tasks</strong> whichwere not  <strong>assigned</strong> to any employee.', 'cpm-pro' ) ?> </p>
    ...
    <div class="cpm-report-page animated" v-if="reportmode" v-animation  >
        <div class="cpm-report-body">
            <div class="cpm-report-filter">
                ...
                <div class=" cpm-no-print cpm-report-btn-pc">
                    ...
                    <a href="#" class="cpm-right  cpm-close" onclick="window.print()"> <img src="<?php echo CPM_URL ?>/assets/images/print.svg" height="16"/></a>
                </div>
                <div class=" cpm-no-print cpm-report-btn-group" >
                    <a href="#" class=" cpm-right button-primary dashicons-before dashicons-media-spreadsheet" @click.prevent="exporttocsv"><?php _e( 'Export to CSV', 'cpm-pro' ) ?></a>
                    <a href="#" class=" cpm-right button-primary dashicons-before dashicons-search" @click.prevent="showmodal"><?php _e( 'Filter Report', 'cpm-pro' ) ?></a>
                </div>

                <div class="clearfix"></div>
            </div>

            <div  v-if="datalist" id="report">
                <div class="cpl-data-head">
                    <div class="cpm-col-3 cpm-border-right first"><b><?php _e( 'Project', 'cpm-pro' ) ?></b> <br/>{{selectedproject}}</div>
                    <div class="cpm-col-3 cpm-border-right second"><b><?php echo _e( 'Coworker', 'cpm-pro' ) ?></b><br/>{{selectedcoworder}}</div>
                    <div class="cpm-col-3 cpm-border-right third"><b><?php _e( 'Total Result', 'cpm-pro' ) ?></b><br/>{{countresult}}</div>
                    <div class="cpm-col-3 fourth">{{{extrahead}}}</div>
                    <div class="clearfix"></div>
                </div>
                    ...
                    <div v-for="project in datalist " class="cpm-report-project">
                        ...
                        <div v-for="list in project.list">
                            ...
                            <table border="0" width="100%" align="center" class="wp-list-table widefat fixed striped posts">
                                <thead>
                                    <tr >
                                        <th width="32%"><?php _e( 'Task', 'cpm-pro' ) ?></th>
                                        <th width="15%"><?php _e( 'Assigned to', 'cpm-pro' ) ?></th>
                                        <th width="12%"><?php _e( 'Assigned Date', 'cpm-pro' ) ?></th>
                                        <th width="12%" v-if="duedate"><?php _e( 'Due Date', 'cpm-pro' ) ?></th>
        ...
        <div v-if="!datalist && !dataloading" >
            <h2><?php _e( 'No Data Found, Please search again!', 'cpm-pro' ) ?></h2>
        </div>
    ...
    <overduemodal :show.sync="showModal" :modalwide="modalwide" :formaction="formaction" :datalist="datalist" :countresult="countresult" :wpnonce="wpnonce">
        <h3 slot="header"><?php _e( 'Report for', 'cpm-pro' ) ?> {{reporttitle}} </h3>
    ...
    <projectonlymodal :show.sync="projectonlyModal" :modalwide="modalwide" :formaction="formaction" :datalist="datalist" :countresult="countresult" :wpnonce="wpnonce">
        ...
        <div slot="task_status">
            <label> <input type="radio" value="-1" name="task_status" checked="checked" />  <?php _e( 'All Task', 'cpm-pro' ) ?> </label> <br/>
            <label>  <input type="radio" value="1" name="task_status"  /> <?php _e( 'All Complete Task', 'cpm-pro' ) ?> </label> <br/>

            <label> <input type="radio" value="0" name="task_status"  />  <?php _e( 'All Incomplete Task', 'cpm-pro' ) ?> </label>
        </div>
            ...
            <div class="modal-wrapper">
                <div class="modal-container" style="width:{{modalwide}}">
                    ...
                    <form @submit.prevent="filterreport('report-run-from')" id='report-run-from' method='post'>
                        ...
                        <slot name='submit'><input type="submit" name="submit" class="button button-primary cpm-doc-btn" value="<?php _e( 'Run Report', 'cpm-pro' ) ?>"    /></slot>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-wrapper">
                <div class="modal-container" style="width:{{modalwide}}">
                    ...
                    <form @submit.prevent="filterreport('report-run-from2')" id='report-run-from2' method='post'>
                        ...
                        <div class="modal-footer">
                            <slot name='submit'>
                                <input type="submit" name="submit" class="button button-primary cpm-doc-btn" value="<?php _e( 'Run Report', 'cpm-pro' ) ?>"    />
                            </slot>
                        </div>
                    </form>
                </div>
                        ...
                        <form @submit.prevent="filterreport('report-ml-from')" id='report-ml-from' method='post'>
                            ...
                            <div class="modal-body">
                                ...
                                <select class="cpm-field" name="project"  required  v-select="selected" >
                                    <option <?php selected( '' ); ?> value="-1"><?php _e( 'Selec a project', 'cpm-pro' ); ?></option>
                                    <?php
