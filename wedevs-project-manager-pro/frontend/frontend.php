...
class CPM_Frontend {
    ...
    function __construct() {
        ...
        add_action( 'cpm_project_id', array( $this, 'get_project_id' ) );
        ...
        add_action( 'wp_footer', array( cpm(), 'admin_js_templates' ) );
        add_filter( 'post_class', array($this, 'category_id_class') );
        add_filter( 'body_class', array($this, 'category_id_class') );
    }

    function get_project_id( $project_id ) {

        return empty( $_GET['project_id'] ) ? $project_id : absint( $_GET['project_id'] ); 
    }

    function settings_section( $section ) {
        $section[] = array(
            'id'    => 'cpm_page',
            'title' => __( 'Page Settings', 'cpm-pro' )
        );
    ...
    function page_settings( $settings_fields ) {
        ...
        $settings_fields['cpm_page'] = array(
            ...
            array(
                'name'    => 'my_task',
                'label'   => __( 'My Task', 'cpm-pro' ),
                'type'    => 'select',
                'options' => $pages_array,
            ),
            array(
                'name'    => 'calendar',
                'label'   => __( 'Calendar', 'cpm-pro' ),
                'type'    => 'select',
                'options' => $pages_array,
            ),
    ...
    function my_task($atts) {
        if ( !class_exists('WeDevs_CPM') ) {
            return __( 'Sorry, the main plugin is not installed', 'cpm-pro');
        }
    ...
    function enqueue_scripts() {
        // add_action( 'admin_enqueue_scripts', array( cpm(), 'admin_scripts' ) );
        // add_action( 'admin_enqueue_scripts', array( 'WeDevs_CPM_Pro', 'calender_scripts' ) );
        // add_action( 'admin_enqueue_scripts', array( 'WeDevs_CPM_Pro', 'my_task_scripts' ) );
        cpm()->admin_scripts();
        WeDevs_CPM_Pro::calender_scripts();
        WeDevs_CPM_Pro::my_task_scripts();
    ...
    function list_projects() {
        include_once CPM_PRO_PATH . '/views/project/project_list.php' ;
    }
    ...
    function single_project( $project_id ) {
        ...
        switch ($tab) {
            ...
            case 'project' :
                  switch ($action) {
                     case 'activity':
                        cpm_get_header( __( 'Activity', 'cpm-pro' ), $project_id );
                        $this->project_activity( $project_id );
                        break;

                    case 'index':
                        cpm_get_header( __( 'Overview', 'cpm-pro' ), $project_id );
            ...
            case 'task':
                CPM_Task::getInstance()->tasks_scripts();
                ...
                switch ($action) {
                    ...
                    default:
                        cpm_get_header( __( 'To-do Lists', 'cpm-pro' ), $project_id );
    ...
    function project_activity( $project_id ) {
        cpm_get_header( __( 'Activities', 'cpm-pro' ), $project_id );
    ...
    function update_notification() {
        ...
        if ( $this->update_check() ) {
            ...
            if ( current_user_can( 'update_core' ) ) {
                $msg = sprintf( __( '<strong>%s</strong> version %s is now available! %s.', 'cpm-pro' ), $version['name'], $version['latest'], $version['msg'] );
            } else {
                $msg = sprintf( __( '%s version %s is now available! Please notify the site administrator.', 'cpm-pro' ), $version['name'], $version['latest'], $version['msg'] );
            }
    ...
    function category_id_class( $classes ) {
        ...
        if(get_the_ID() == cpm_get_option('project', 'cpm_page')){
            $classes[] = 'cpm-project cpm-'.$template;
        } else if( get_the_ID() == cpm_get_option('my_task', 'cpm_page') ) {
            $classes[] = 'cpm-mytask cpm-'.$template;
        }else if( get_the_ID() == cpm_get_option('calendar', 'cpm_page') ) {
            $classes[] = 'cpm-calendar cpm-'.$template;
        }
