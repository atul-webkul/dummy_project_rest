...
function bm_send_email() {
	...
	if (isset($_POST['responder_bid_response'])) {
		...
		$bm_responder = $_POST['responder_busname'];
		 ...
		 $query = "SELECT bmuser_poc, bmuser_email, job_name FROM " . BM_USER . " LEFT OUTER JOIN " . BM_BIDS . " ON " . BM_USER . ".id=" . BM_BIDS . ".bmuser_id WHERE " . BM_BIDS . ".bid_id = %d"; 
		...
		//$userinfo = get_userdata($record->bmuser_id);
		//$to= $userinfo->user_email;
		
		$to = stripslashes($record->bmuser_email);
		$bm_job_name = stripslashes($record->job_name);
		$bm_poc = stripslashes($record->bmuser_poc);
		//$bm_poc = $userinfo->user_login;
		...
		$message .= '<p>To login and review, please <a href="' . BM_SITE_URL . '/my-account">click here</a>.</p>';

		$headers   = array(
			'Content-Type: text/html; charset=UTF-8',
			'From: Response from bidder <bids@caribbeanhomenetwork.com>'
		);
...
function bm_update_bid() {
	...
	$bm_file_path = bm_handle_file_upload('responder_bid_file', 'bid_responses_new/');

	// Insert the record into the responses table
	$query = "INSERT INTO " . BM_BIDS_RESPONSES . " (bid_id, bider_id , responder_busname, responder_poc, responder_phone, responder_email, responder_bid_file, responder_notes, date_submitted, bid_accepted, quoted_total)" .
		"VALUES (%d, %d, %s, %s, %s, %s, %s, %s, %s, %d, %s);";
	$data = array(
		$bm_bid_id,
		$bider_id,
...
function bm_responder_responses() {
	...
	$query = "SELECT * FROM " . BM_BIDS . " LEFT OUTER JOIN " . BM_USER . " ON " . BM_BIDS . ".bmuser_id=" . BM_USER . ".id WHERE bid_id = %d";
	$data = array($recordId);
	$query = $wpdb->prepare($query, $data);
